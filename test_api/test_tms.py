...
def test_add_board_members(token, tms_service):
    ...
    response_json = tms_service.add_user_board(board_id, 2, 201)
...
def test_get_board_members(token, tms_service):
    response_json = tms_service.get_board_members(1, 200)

    logger.info(response_json)
    assert response_json[0]["id"] == 2
    assert response_json[0]["username"] == "user1"
    assert response_json[0]["email"] == "user1@example.com"


@pytest.mark.only
def test_get_board_stats(token, tms_service):
    response_json = tms_service.get_board_stats(1, 200)
