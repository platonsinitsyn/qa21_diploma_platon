...
def check_board_access(board, user_id: int, user_role: str, action: str = "read"):
    ...
    if user_role == "user" and action != "read":
        if board.created_by != user_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="You don't have permission to modify this board"
            )
        return True
    ...
    if user_role == "guest":
        if action != "read":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Guests can only view public boards")
        if not board.public:
            raise HTTPException(
