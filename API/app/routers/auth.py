...
def register_admin(admin_data: RegisterAdminRequest, db: Session = Depends(get_db)):
    """
    ...
    if user_count > 0:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Admin already exists. Registration is disabled."
        )

    # Создание первого администратора
    from app.schemas.user import UserCreate

    user_data = UserCreate(username=admin_data.username, email=admin_data.email, password=admin_data.password)
...
def register_user(user_data: RegisterAdminRequest, db: Session = Depends(get_db)):
    """
    ...
    new_user_data = UserCreate(username=user_data.username, email=user_data.email, password=user_data.password)
...
def register_guest(user_data: RegisterAdminRequest, db: Session = Depends(get_db)):
    """
