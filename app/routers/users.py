...
from app.schemas.user import AvatarUpdate, PasswordUpdate, UserResponse
...
def get_users(
    skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)
):
    """
...
def get_my_tasks(
    ...
    tasks = db.query(Task).filter(Task.created_by == current_user_id).offset(skip).limit(limit).all()
...
def get_user_avatar(user_id: int, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)):
    """
