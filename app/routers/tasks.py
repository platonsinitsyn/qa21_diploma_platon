...
from app.schemas.task import BulkDelete, BulkStatusUpdate, ReorderTasks, TaskCreate, TaskResponse, TaskUpdate
...
def get_tasks(
    ...
    tasks = task_service.get_tasks_by_board(
        db, board_id, status_filter=status_filter, priority_filter=priority_filter, skip=skip, limit=limit
    )
    return tasks
...
def get_task(
    board_id: int, task_id: int, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)
):
    """
...
def update_task_status(
    task_id: int, new_status: str, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)
):
    """
...
def update_task_to_next_status(
    task_id: int, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)
):
    """
...
def update_task_priority(
    task_id: int, new_priority: str, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)
):
    """
