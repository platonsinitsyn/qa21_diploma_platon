...
def move_task(db: Session, task_id: int, target_board_id: int) -> Task:
    ...
    if not target_board:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Target board not found")
...
def update_task_status(db: Session, task_id: int, new_status: str) -> Task:
    ...
    if new_status not in valid_statuses:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid status. Must be one of: {valid_statuses}"
        )
...
def get_next_status(current_status: str) -> str:
    ...
    status_flow = {"todo": "in_progress", "in_progress": "done", "done": "done"}  # Остаётся на done
...
def update_task_priority(db: Session, task_id: int, new_priority: str) -> Task:
    ...
    if new_priority not in valid_priorities:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid priority. Must be one of: {valid_priorities}"
        )
...
def search_tasks(db: Session, query: str, skip: int = 0, limit: int = 100) -> List[Task]:
    ...
    tasks = (
        db.query(Task)
        .filter(or_(Task.title.ilike(search_term), Task.description.ilike(search_term)))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def bulk_update_status(db: Session, task_ids: List[int], new_status: str) -> int:
    ...
    updated = db.query(Task).filter(Task.id.in_(task_ids)).update({"status": new_status}, synchronize_session=False)
    db.commit()
