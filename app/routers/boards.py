...
from app.core.security import check_board_access, get_current_user_id
...
def create_board(
    board_data: BoardCreate, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)
):
    """
    Создать новую доску.
    Требуется аутентификация.
...
def get_board(board_id: int, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)):
    """
...
def delete_board(board_id: int, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)):
    """
    Удалить доску.
...
def add_board_member(
    board_id: int, user_id: int, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)
):
    """
    ...
    if user.role != "admin" and board.created_by != current_user_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only board owner or admin can add members")
...
def remove_board_member(
    ...
    if user.role != "admin" and board.created_by != current_user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only board owner or admin can remove members"
        )
...
def get_board_members(
    board_id: int, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)
):
    """
...
def archive_board(board_id: int, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)):
    """
    Архивировать доску.
    ...
    user = user_service.get_user_by_id(db, current_user_id)
    if user.role != "admin" and board.created_by != current_user_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only board owner or admin can archive board")
...
def get_board_stats(board_id: int, db: Session = Depends(get_db), current_user_id: int = Depends(get_current_user_id)):
    """
